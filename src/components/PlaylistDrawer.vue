...
// Cover is already a blob URL or http URL after preconvertCover in import/restore.
// No conversion needed here. Just pass through.
</script>
  ...
  <Transition name="drawer-spring">
    <div v-if="player.showPlaylist" class="absolute top-0 right-0 bottom-0 w-80 bg-cosmos-950/95 backdrop-blur-xl border-l border-white/10 z-40 flex flex-col shadow-[-10px_0_30px_rgba(0,0,0,0.5)]">
      ...
      <div class="p-5 border-b border-white/5 flex justify-between items-center bg-black/20">
          ...
          <button @click="player.togglePlaylist" class="text-white/50 hover:text-white hover:rotate-90 active:scale-75 transition-all duration-400 ease-[cubic-bezier(0.2,0.8,0.2,1)] no-outline">x</button>
      </div>

      <RecycleScroller
        class="flex-1 scrollbar-hide"
        :items="player.queue"
        :item-size="60"
        key-field="id"
        :buffer="300"
      ...
      >
        <div class="px-3 pt-1">
          <div
            ...
            class="track-item flex items-center gap-3 p-3 rounded-lg cursor-pointer group"
          ...
          >
            ...
            <button @click.stop="player.toggleLike(track)" class="text-white/50 hover:text-red-500 hover:scale-125 active:scale-75 mr-2"

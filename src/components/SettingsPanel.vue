...
<template>
  <Transition name="panel-fade">
    <div class="absolute inset-0 z-30 flex bg-cosmos-950/80 backdrop-blur-xl">
      <div class="w-64 h-full bg-black/40 flex flex-col p-6 z-10 border-r border-white/5 shadow-2xl">
        ...
        <Transition name="slide-up-fade" mode="out-in">
          ...
          <div v-else-if="activeSettingTab === 'audio'" class="h-full overflow-y-auto scrollbar-hide max-w-4xl mx-auto">
              ...
              <div class="grid grid-cols-3 gap-4">
                  <button @click="setChannel(2)" class="p-6 rounded-2xl border flex flex-col items-center gap-4 transition-all duration-400 ease-[cubic-bezier(0.2,0.8,0.2,1)] active:scale-95 no-drag-btn outline-none" :class="currentChannel === 2 ? 'bg-starlight-cyan/15 border-starlight-cyan text-white shadow-[0_0_20px_rgba(100,255,218,0.15)] scale-105' : 'bg-white/5 border-white/5 text-white/40 hover:bg-white/10 hover:scale-[1.02]'"><Speaker :size="32" class="transition-transform duration-400" :class="currentChannel === 2 ? 'scale-110' : ''" /><span class="font-bold tracking-widest text-xs">STEREO (2.0)</span></button>
                  <button @click="setChannel(6)" class="p-6 rounded-2xl border flex flex-col items-center gap-4 transition-all duration-400 ease-[cubic-bezier(0.2,0.8,0.2,1)] active:scale-95 no-drag-btn outline-none" :class="currentChannel === 6 ? 'bg-starlight-cyan/15 border-starlight-cyan text-white shadow-[0_0_20px_rgba(100,255,218,0.15)] scale-105' : 'bg-white/5 border-white/5 text-white/40 hover:bg-white/10 hover:scale-[1.02]'"><Speaker :size="32" class="transition-transform duration-400" :class="currentChannel === 6 ? 'scale-110' : ''" /><span class="font-bold tracking-widest text-xs">SURROUND (5.1)</span></button>

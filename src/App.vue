  ...
  (newTrack) => {
    if (newTrack && newTrack.id) {
      ...
      invoke('sync_smtc_metadata', {
        title: newTrack.title || 'Unknown',
        artist: newTrack.artist || 'Unknown',
        cover: newTrack.cover || ''
      }).catch(e => console.error("SMTC Metadata Error:", e));

      // 强行同步播放状态，防止系统 UI 卡顿
      invoke('sync_smtc_status', { isPlaying: player.isPlaying })
        .catch(e => console.error("SMTC Status Error:", e));
    }
  }, 
...
watch(
  () => player.isPlaying, 
  (newVal) => {
    invoke('sync_smtc_status', { isPlaying: newVal })
      .catch(e => console.error("SMTC Status Error:", e));
  }, 
  { immediate: true }
);
...
onMounted(async () => { 
  // 监听来自系统全局的媒体按键
  await listen('smtc-toggle', () => player.togglePlay());
  await listen('smtc-next', () => player.nextTrack());
  await listen('smtc-prev', () => player.prevTrack());

  notify('ASTRAL_SYSTEM ONLINE'); 
  player.setNotifier(notify); 
  
  // 执行核心引擎自检 (修复首播无声的关键)
  await player.initCheck(); 
  player.fetchDevices();
...
<template>
  <main class="relative flex w-screen h-screen overflow-hidden text-cosmos-100 bg-[#05080a] font-sans rounded-xl border border-white/10">
    ...
    <div 
      class="absolute top-[-15%] right-[-10%] w-[600px] h-[600px] rounded-full pointer-events-none z-0 animate-float-slow opacity-70" 
      style="background: radial-gradient(circle at 30% 30%, rgba(189, 52, 254, 0.4) 0%, rgba(80, 20, 120, 0.1) 60%, transparent 100%); box-shadow: inset -20px -20px 50px rgba(0,0,0,0.5); filter: blur(40px);"
    ></div>
    
    <div 
      class="absolute bottom-[-20%] left-[-15%] w-[700px] h-[700px] rounded-full pointer-events-none z-0 animate-float-slower opacity-60" 
      style="background: radial-gradient(circle at 70% 30%, rgba(100, 255, 218, 0.3) 0%, rgba(20, 120, 100, 0.05) 60%, transparent 100%); box-shadow: inset 20px 20px 50px rgba(0,0,0,0.5); filter: blur(50px);"
    ></div>
    ...
    <div class="relative z-10 flex w-full h-full backdrop-blur-[1px]">
      ...
      <section class="flex flex-col flex-1 relative z-20">
        <header class="h-16 flex items-center justify-between px-8 border-b border-white/5 bg-cosmos-900/20 cursor-move" data-tauri-drag-region>
          <div class="text-xs font-mono tracking-[0.3em] text-starlight-cyan/50 pointer-events-none opacity-50">
            /// ASTRAL_CORE_V1
          </div>
          ...
          <div class="flex gap-3">
            <button 
              @click="closeWindow" 
              class="w-3.5 h-3.5 rounded-full bg-red-500/20 border border-red-500/50 hover:bg-red-500 transition-all duration-300 ease-[cubic-bezier(0.2,0.8,0.2,1)] hover:scale-125 active:scale-90 flex items-center justify-center group no-drag-btn no-outline"
            >
              <span class="opacity-0 group-hover:opacity-100 text-[8px] text-white">✕</span>
            </button>
            <button 
              @click="minimize" 
              class="w-3.5 h-3.5 rounded-full bg-yellow-500/20 border border-yellow-500/50 hover:bg-yellow-500 transition-all duration-300 ease-[cubic-bezier(0.2,0.8,0.2,1)] hover:scale-125 active:scale-90 flex items-center justify-center group no-drag-btn no-outline"
            >
              <span class="opacity-0 group-hover:opacity-100 text-[8px] text-white">−</span>
            </button>
            <button 
              @click="toggleMaximize" 
              class="w-3.5 h-3.5 rounded-full bg-green-500/20 border border-green-500/50 hover:bg-green-500 transition-all duration-300 ease-[cubic-bezier(0.2,0.8,0.2,1)] hover:scale-125 active:scale-90 flex items-center justify-center group no-drag-btn no-outline"
            >
        ...
        <div class="flex-1 relative overflow-hidden w-full">
          ...
          <Transition name="page-spring">
              <div v-if="activeTab === 'likes'" class="absolute inset-0 z-20 flex flex-col p-10 overflow-y-auto scrollbar-hide">
                 <h2 class="text-4xl font-bold font-orbitron text-white mb-8 flex items-center gap-4">
                    <Heart :size="32" class="text-red-500 fill-red-500" /> 
                    LIKED TRACKS
                 </h2>
                 <div class="grid grid-cols-1 gap-2">
                    <div 
                      v-for="track in player.likedQueue" 
                      :key="track.id" 
                    ...
                    >
                       ...
                       <button 
                         @click.stop="player.toggleLike(track)" 
                         class="text-red-500 hover:scale-125 active:scale-75 transition-all duration-300 ease-[cubic-bezier(0.2,0.8,0.2,1)]"
                       >
                          <Heart :size="20" class="fill-red-500"/>
                       </button>
                    </div>
                    
                    <div v-if="player.likedQueue.length === 0" class="text-white/30 text-center mt-20">
                      NO LIKED TRACKS YET
                    </div>
                 </div>
              </div>
          ...
          <div 
            v-show="activeTab === 'dashboard' || activeTab === 'settings'" 
...
button, input, [role="button"], .no-drag-btn { 
